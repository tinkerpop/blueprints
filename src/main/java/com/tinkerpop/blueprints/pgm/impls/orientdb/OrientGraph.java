...
public class OrientGraph implements Graph, TransactionalGraph {
	...
	public Vertex addVertex(final Object iId, final String iClassName) {
		try {
			beginTransaction();

			final OrientVertex vertex = new OrientVertex(this, this.database.createVertex(iClassName));
			vertex.save();
	...
	public Vertex addVertex(final Object iId) {
		return addVertex(iId, null);
	}
	...
	protected void putElementInCache(final OrientElement iElement) {
		final ODocument d = iElement.getRaw().getDocument();

		if (this.useCache && d.getIdentity().isValid()) {
			if (cachedClasses.contains(d.getClassName()))
				this.rid2Elements.put(d.getIdentity(), iElement);
		}
	}
	...
	/*
	 * public void startStopTransaction() { if (Mode.AUTOMATIC == this.mode) { database.commit(); database.begin(); } }
	 */
