 ...
 *
 * If the number of elements in an Iterable is known, then the Iterable can be wrapped
 * in this class to make determining the size a constant time operation rather than
 * an iteration over the whole Iterable.
 *
 * Some generators need to know the number of vertices prior to generation. Using this
 * class can speed up the generator.
 * 
...
public class SizableIterable<T> implements Iterable<T> {
     ...
     *
     * Checks if the given Iterable is an instance of a Collection or SizableIterable before
