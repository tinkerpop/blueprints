...
import com.tinkerpop.blueprints.pgm.util.wrappers.event.StubGraphChangedListener;
...
public class EventTransactionalGraphTest extends GraphTest {
    ...
    public void testEventedGraph() {
        graph.addListener(new ConsoleGraphChangedListener(graph));
        assertTrue(graph.getVertices() instanceof EventTransactionalVertexIterable);
        assertTrue(graph.getEdges() instanceof EventTransactionalEdgeIterable);
    ...
    public void testEventedElement() {
        ...
        for (Vertex vertex : graph.getVertices()) {
            ...
            assertTrue(vertex.getOutEdges() instanceof EventTransactionalEdgeIterable);
            assertTrue(vertex.getInEdges() instanceof EventTransactionalEdgeIterable);
            assertTrue(vertex.getOutEdges("knows") instanceof EventTransactionalEdgeIterable);
            assertTrue(vertex.getInEdges("created") instanceof EventTransactionalEdgeIterable);
        }

        for (Edge edge : graph.getEdges()) {
            assertTrue(edge instanceof EventTransactionalEdge);
            edge.removeProperty("weight");
            assertNull(edge.getProperty("weight"));

            assertTrue(edge.getOutVertex() instanceof EventTransactionalVertex);
            assertTrue(edge.getInVertex() instanceof EventTransactionalVertex);
        }
    }
    ...
    public void testTransactionSeriesWithFailure(){
        ...
        ((EventTransactionalGraph) graph).stopTransaction(TransactionalGraph.Conclusion.FAILURE);
