...
public class CSVReaderTestSuite extends TestSuite {

    public CSVReaderTestSuite() {
    }

    public CSVReaderTestSuite(GraphTest graphTest) {
        super(graphTest);
    }
    ...
    public void testReadingTinkerGraph() throws Exception {
        ...
        // note that CSV does not have the notion of Edge Identifiers built into the specification
        ...
        if (!graph.getFeatures().ignoresSuppliedIds) {
            ...
            for (Edge e : graph.getVertex("1").getEdges(Direction.OUT)) {
                if (e.getVertex(Direction.IN).getId().equals("2")) {
                    ...
                    assertEquals(0, Math.round(Float.parseFloat((String) e.getProperty("weight"))));
                    assertEquals(e.getLabel(), "created");
                    counter++;
                } else if (e.getVertex(Direction.IN).getId().equals("4")) {
                    assertEquals(1, Math.round(Float.parseFloat((String) e.getProperty("weight"))));
                    assertEquals(e.getLabel(), "knows");
                    counter++;
                }
            }
