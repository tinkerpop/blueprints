 ...
 * Labels are managed as OrientDB classes.
 * 
...
public abstract class OrientElement implements Element, OSerializableStream,
	...
	protected OrientElement(final OrientBaseGraph rawGraph,
			final OIdentifiable iRawElement) {
		graph = rawGraph;
	...
	public <T extends OrientElement> T setProperties(final Object... fields) {
		if (fields != null && fields.length > 0 && fields[0] != null) {
			...
			if (fields.length == 1) {
				Object f = fields[0];
				if (f instanceof Map<?, ?>) {
					for (Map.Entry<Object, Object> entry : ((Map<Object, Object>) f)
							.entrySet())
						setPropertyInternal(this,
								(ODocument) rawElement.getRecord(), entry
	...
	@Override
	public OSerializableStream fromStream(final byte[] iStream)
	...
	protected void checkClass() {
		...
		if (cls == null || !cls.isSubClassOf(getBaseClassName()))
			throw new IllegalArgumentException(
					"The document received is not a " + getElementType()
							+ ". Found class '" + cls + "'");
	}

	/**
	 * Check if a class already exists, otherwise create it at the fly. If a
	 * transaction is running commit changes, create the class and begin a new
	 * transaction.
	 * 
	 * @param iClassName
	 *            Class's name
	...
	protected String checkForClassInSchema(final String iClassName) {
		...
		if (!schema.existsClass(iClassName)) {
				...
				graph.executeOutsideTx(
						new OCallable<OClass, OrientBaseGraph>() {
							...
							public OClass call(final OrientBaseGraph g) {
								return schema.createClass(iClassName,
										schema.getClass(getBaseClassName()));
			...
			if (!cls.isSubClassOf(getBaseClassName()))
				throw new IllegalArgumentException("Class '" + iClassName
						+ "' is not an instance of " + getBaseClassName());
		}
